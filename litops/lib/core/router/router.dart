...
import '../../features/admin/screens/pending_profiles_screen.dart';
...
final goRouterProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text('Page Not Found', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white)),
            ...
            ElevatedButton(
              ...
              child: Text(ref.read(authStateProvider).isAuthenticated ? 'Go Home' : 'Go to Login'),
            ),
          ],
        ),
      ),
    ...
    redirect: (context, state) {
      ...
      debugPrint('[ROUTER] Redirecting from: $location, isLoggedIn: $isLoggedIn');

      // Prevent eager redirects while profile is still loading asynchronously
      if (isLoading && location != '/splash') {
        debugPrint('[ROUTER] Loading in progress, not redirecting');
        return null; 
      }
      ...
      // Profile setup redirect removed per user request. Users go straight to dashboard.
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (context, state) {
              ...
              return RulebookViewerScreen(fileUrl: fileUrl);
            },
          ),
          GoRoute(
            path: '/search',
