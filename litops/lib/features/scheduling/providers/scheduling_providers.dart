...
class SchedulingController {
  ...
  Future<void> assignMember({
    ...
    // Safety net: create notification for the ASSIGNED USER
    // The DB trigger should also do this, but this covers the case
    // where the trigger hasn't been deployed yet.
    try {
      // Fetch event name for the notification message
      ...
      final adminData = await SupabaseConfig.client
          ...
          .eq('id', assignedBy)
          .single();
      final adminName = adminData['full_name'] ?? 'Admin';

      await SupabaseConfig.client.from(SupabaseTables.notifications).insert({
        'user_id': userId,  // ← recipient = assigned user, NOT admin
        'title': 'New Event Assignment',
        'message': 'You have been assigned to the event "$eventName" by $adminName.',
        'event_id': eventId,
      });
    } catch (_) {
      // DB trigger may have already created the notification — safe to ignore
    }
  ...
  Future<void> removeAssignment(String assignmentId, String eventId) async {
    // Fetch assignment details before deleting (need user_id for notification)
    ...
    try {
      final assignment = await SupabaseConfig.client
          ...
          .eq('id', assignmentId)
          .single();
      removedUserId = assignment['user_id'] as String?;
    } catch (_) {}
    ...
    // Safety net: create unassignment notification
