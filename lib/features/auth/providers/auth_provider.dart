...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> signIn(String email, String password, {bool rememberMe = false}) async {
    ...
    try {
      ...
      if (email == _superAdminEmail && password == _superAdminPassword &&
        ...
        try {
          await _authRepository.signUp(email, password, 'Super Admin', DateTime(1990, 1, 1));
          final session = SupabaseConfig.client.auth.currentSession;
          if (session == null) {
  ...
  Future<void> signUp(String email, String password, String fullName, DateTime dob) async {
    state = state.copyWith(isLoading: true, error: null);
    try {
      await _authRepository.signUp(email, password, fullName, dob);

      // Eagerly create the profile row if a session was created
      final session = SupabaseConfig.client.auth.currentSession;
      if (session != null) {
        final userId = session.user.id;
        final dobStr = dob.toIso8601String().split('T').first;
        final isAdmin = email == _superAdminEmail;
        final roleStr = isAdmin ? 'super_admin' : 'junior_wing';

        try {
          await SupabaseConfig.client.from(SupabaseTables.profiles).upsert({
            'id': userId,
            'email': email,
            'full_name': fullName,
            'role': roleStr,
          ...
          // RLS may block — profile will be created on first sign-in
        }

        // Sign out so the user is NOT auto-logged-in after registration
        try {
